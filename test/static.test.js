...
assert.match(css, /grid-template-rows:\s*auto\s*auto/);
...
assert.match(css, /\.deck-area\s*{[\s\S]*min-height:\s*min\(350px,\s*44svh\)/);
assert.match(css, /\.deck\s*{[\s\S]*width:\s*min\(178px,\s*47vw\)/);
